---@param cam_handler number|nil
---@param coords vector3|nil
---@param options {onupdate: function, destructor: function, accept: function}
---@return integer|nil
function StartFreecam(cam_handler,coords,options)
    local cam = SetFreecamActive(true,coords,options,cam_handler)
    StartFreecamThread(coords)
    return cam
end
...
local function GetSpeedMultiplier()
    ...
    local baseSpeed = ScriptData.Cam_Speed --SETTINGS.BASE_MOVE_MULTIPLIER
...
function StartFreecamThread(coords)
...
function GetFreecamPosition()
    return _internal_pos or C_ZERO_VEC3
end
...
function GetFreecamRotation()
    return _internal_rot or C_ZERO_VEC3
end
...
function SetFreecamActive(active, coords, options, cam_handler)
    ...
    if active then
        ...
        local rot = cam_handler and GetCamRot(cam_handler, C_ROT_ORDER) or GetInitialCameraRotation()

        _internal_camera = cam_handler or CreateCam('DEFAULT_SCRIPTED_CAMERA', true)
        _internal_acc = options.accept
        _internal_bac = options.destructor
        _internal_onupdate = options.onupdate

        SetFreecamFov(C_CAM_DEFAULT_FOV)
        SetFreecamPosition(pos.x, pos.y, pos.z)
        SetFreecamRotation(rot.x, rot.y, rot.z)
    else
